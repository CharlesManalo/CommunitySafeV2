  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // For students, redirect to hazard reporting after 2 seconds
        if (userType === "student") {
          setTimeout(() => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-8">
      ...
      >
        ...
        <p className="text-slate-400">Please confirm the information below</p>
      </motion.div>
      ...
      >
        <div className="bg-slate-800/50 backdrop-blur-sm rounded-2xl p-8 border border-slate-700/50">
          ...
          <div className="flex items-center gap-4 mb-6 pb-6 border-b border-slate-700/50">
            ...
            >
              <User
                className={`w-6 h-6 ${isTeacher ? "text-purple-400" : "text-blue-400"}`}
              />
            </div>
            <div>
              <p className="text-slate-400 text-sm">User Type</p>
              <p
                className={`text-lg font-semibold ${isTeacher ? "text-purple-300" : "text-blue-300"}`}
              >
                {displayName}
              </p>
            </div>
        ...
        <div className="mt-4 p-4 bg-slate-800/30 rounded-xl border border-slate-700/30">
          <p className="text-slate-400 text-xs text-center">
            <span className="text-emerald-400">Privacy Protected:</span> Your
            full card details are encrypted and stored securely. Only authorized
